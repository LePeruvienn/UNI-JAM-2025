...
public class Audience : MonoBehaviour
{
    ...
    private void OnEnable()
    {
        ...
        if (gameManager == null)
        {
            Debug.LogWarning("[AudienceManager] GameManager introuvable : impossible de s'abonner aux �v�nements.");
            return;
        }
    ...
    private void HandleSlideSuccess(Rule.ActionType action)
    {
        if (animator != null)
        {
            switch (action)
            {
                ...
                case Rule.ActionType.RaiseHands:
                    ...
                    break;
                default:
                    Debug.LogWarning("[AudienceManager] Action inconnue re�ue pour success: " + action);
                    break;
            }
            return;
        }
        ...
        {
            Debug.LogWarning("[AudienceManager] Animator manquant � impossible d'afficher l'animation de succ�s.");
            switch (action)
            {
                case Rule.ActionType.Clap:
                    PlayClip(clapClip);
